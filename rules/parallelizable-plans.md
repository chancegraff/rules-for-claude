...
Implementation plans must be parallelizable. The lead is the orchestrator dispatching parallel agents, one per stream the plan names; a non-parallelizable plan wastes that capability and stalls the work. When designing any non-trivial plan's wave structure, ask: can the streams be implemented at the same time without colliding? If the honest answer requires serial dependencies, restructure into:
