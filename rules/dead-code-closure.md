...
After the deletion, run the repo's unused-export check to catch what the walk missed: `import/no-unused-modules` where the repo's ESLint config already has it, `knip` otherwise. Stories and decorators are caught only when the tool is configured to see them.
